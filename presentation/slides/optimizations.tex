...
\node[rect] at (0,2) (4) {\textbf{Jacobian coordinates} - Algorithmic changes, jacobian coordinates};
\node[rect] at (0,0) (5) {\textbf{Final} - Final performance optimization };
...
\begin{itemize}
\setlength\itemsep{0.5em}
\item{Change the base from 2 bit to 64 bit for Montogomery}
\item{Precomputation of $2^{k}\cdot G$ where $k \in \{1, ..., \log_2(p)\}$ and $G \in E$}
\item{Vectorize shifting using AVX2}
\end{itemize}
}
\item \textbf{Jacobian coordinates}

\item{
\textbf{Final optimization}

\begin{itemize}
	\setlength\itemsep{0.5em}
\item{Function stiching}
...
x86 icc 13.0.1 -m64 -march=xCORE-AVX2 -O3
